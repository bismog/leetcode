...
# Source image file should be restricted to name: daisy-ubase.tar.gz
_source_img="./daisy-ubase.tar.gz"
_source_spec="./daisy-ubase.spec"
_version="20170801"
_release="babylon"
...
if [[ $# -ne 0 ]];then
    ...
    do
        ...
        -* | --* | -h | --help | help)
        echo "Command usage:"
        echo "    -i    Path to image file(daisy-ubase.tar.gz)"
        echo "    -c    Path to spec file(daisy-ubase.spec)"
        echo "    -v    version item mapping to rpm file"
        echo "    -r    release item mapping to rpm file"
        echo "e.g.:"
        echo "    $me -i path/to/daisy-ubase.tar.gz -c /path/to/spec"
        exit 1
        ;;
...
if [[ $? -eq 0 ]];then
    # rsync ../RPMS/x86_64/daisy-ubase-$_VERSION-$_RELEASE.x86_64.rpm root@cml04:/data/yum.repo/daisy/daisy-compile/
    # ssh root@cml04 'cd /data/yum.repo/daisy/daisy-compile/;createrepo -p --update -o . .'

    rsync ../RPMS/x86_64/daisy-ubase-$_VERSION-$_RELEASE.x86_64.rpm root@repobase:/var/www/html/repo/yum/daisy/
